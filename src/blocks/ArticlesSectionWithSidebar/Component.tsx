...
interface ArticlesSectionWithSidebarBlockProps {
  ...
  featuredOnLogos?:
    | { logo: MediaType | string; name?: string | null; link?: string | null }[]
    | null
...
export const ArticlesSectionWithSidebarBlock: React.FC<
  ArticlesSectionWithSidebarBlockProps
> = async (props) => {
  const {
    // About section
    showAbout = true,
        ...
        <div className={`flex flex-col ${showSidebar ? 'lg:flex-row gap-8' : ''}`}>
              ...
              <div className="sticky top-4 space-y-6">
                ...
                {agent && (
                  <div className="text-center">
                    ...
                    {/* Tagline/Credentials */}
                    {agent.tagline && <p className="text-sm text-gray-600 mb-3">{agent.tagline}</p>}
                    ...
                    {showSocialLinks && agent.socialLinks && (
                      <div className="flex flex-wrap justify-center gap-2 mb-4">
                        ...
                        {agent.socialLinks.pinterest && (
                          <SocialLink
                            href={agent.socialLinks.pinterest}
                            icon="pinterest"
                            color="#bd081c"
                          />
                        )}
                ...
                {showContactInfo && agent && (
                  <div className="bg-gray-100 rounded-lg p-6">
                    ...
                    <p className="text-sm text-gray-600 mb-4">
                      Contact details for {agent.displayName || agent.name} and{' '}
                      {agent.displayName ? 'his' : 'their'} real estate business are as follows:
                    </p>
                    <div className="space-y-2 text-sm">
                      ...
                      {(agent as any).website && (
                        <div>
                          ...
                          <a
                            href={
                              (agent as any).website.startsWith('http')
                                ? (agent as any).website
                                : `https://${(agent as any).website}`
                            }
                            target="_blank"
                      ...
                      {agent.socialLinks?.googleMaps && (
                        <div>
                          ...
                          <a
                            href={agent.socialLinks.googleMaps}
                            target="_blank"
                            rel="noopener noreferrer"
                ...
                {showFeaturedOn && serializedLogos.length > 0 && (
                  <div>
                    <h3
                      className="inline-block text-sm font-bold text-gray-900 px-2 py-1 mb-4"
          ...
          <div className={showSidebar ? 'lg:w-2/3 order-1 lg:order-2' : 'w-full'}>
            ...
            {enablePagination ? (
              ...
              <div
                className={`grid ${columns === '1' ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'} gap-8`}
              >
